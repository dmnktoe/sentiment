...
interface AltchaStateChangeEvent extends Event {
  detail: {
    payload?: string;
    state?: 'unverified' | 'verifying' | 'verified' | 'error' | 'expired';
  };
}

type AltchaWidgetElement = HTMLElement & { reset?: () => void };
...
export function NewsletterForm() {
  ...
  useEffect(() => {
    ...
    const widget = document.querySelector<AltchaWidgetElement>('altcha-widget');
    if (!widget) return;
  ...
  const onSubmit = async (data: NewsletterSubscribeInput) => {
    ...
    try {
      ...
      document.querySelector<AltchaWidgetElement>('altcha-widget')?.reset?.();
    } catch (error) {
      setStatus('error');
      setMessage(
